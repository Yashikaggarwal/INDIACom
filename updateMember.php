...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // SQL query to update the member record
    $sql = "UPDATE user SET Name=?, Address=?, Country=?, State=?, Pincode=?, `Mobile`=?, Email=?, `Interested Event`=?, `CSI Membership No.`=?, `IEEE/ISTE/IETE/IITP/IMP Membership No.`=?, Organisation=?, Category=?, Password=? WHERE MemberID=?";

    // Prepare the statement
    $stmt = $conn->prepare($sql);

    // Bind parameters
    $stmt->bind_param("ssssissssssssi", $name, $address, $country, $state, $pincode, $mobile, $email, $interested_event, $csi_membership_no, $ieee_membership_no, $organisation, $category, $password, $member_id);

    // Execute the statement
    if ($stmt->execute()) {
        echo "<script type='text/javascript'>
                alert('Member record updated successfully.');
                window.location.href = 'adminHomeMember.php';
              </script>";
    }  else {
...
//     // SQL query to select member details
//     $sql = "SELECT * FROM user WHERE MemberID = ?";
...
<head>
    ...
    <link rel="stylesheet" href="style.css" href="signup.css">
...
<body>
    ...
    <div class="container">
        ...
        <select id="interested_event" name="interested_event" required>
            ...
            <label for="csi_membership_no">CSI Membership No.:</label>
            <input type="text" id="csi_membership_no" name="csi_membership_no"><br><br>

            <label for="ieee_membership_no">IEEE Membership No.:</label>
            <input type="text" id="ieee_membership_no" name="ieee_membership_no"><br><br>
